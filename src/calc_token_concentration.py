...
logger = init_logger(__name__, log_file_path=Path("logs/calc_token_concentration.log"))
fix_seeds(0)
...
def main() -> None:
    ...
    logger.info(
        ...
        "for embedding output and all layer outputs..."
    )
        ...
        for hs_idx, hidden_states in enumerate(hidden_states_tuple):
            ...
            for b_idx, text_id in enumerate(batch_text_ids):
                ...
                sx_over_mu_sq = torch.where(
                    mu_sq > 0,
                    s_x / mu_sq,
                    torch.tensor(0.0, device=mu_sq.device),
                )
